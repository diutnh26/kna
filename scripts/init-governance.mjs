 ...
 * Run from app/: node ../scripts/init-governance.mjs
 */
import fs from "fs";
...
const evidence = {
  ...
  deployTx: "2zu5VkBki6q9hmYUyiVDepS5KqjrZUsRRJydtDKd1s5AX5mSCu5RUuZngR9tYYRTUaM9CmkCkmS4GkTLJ1z8vCMa",
...
if (!info) {
  evidence.txs.initialize = await send(
  ...
  if (process.env.KNA_COMMITTEE_VAULT) {
    evidence.txs.setVault = await send(
...
try {
  evidence.txs.grantCoordinator = await send(
    [buildGrantCoordinatorIx(deploy.publicKey, coordinator.publicKey)],
    [deploy]
  );
} catch (err) {
  console.log("grant_role skipped/failed (maybe exists):", err.message || err);
}
