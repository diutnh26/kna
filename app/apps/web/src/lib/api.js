...
export const api = {
  ...
  chainPrepare: (id, body, token) =>
    request(`/chain/ledger/${id}/prepare`, { method: 'POST', body, token }),
  chainSubmit: (id, body, token) =>
    request(`/chain/ledger/${id}/submit`, { method: 'POST', body, token }),
  chainFinalize: (id, body, token) =>
    request(`/chain/ledger/${id}/finalize`, { method: 'POST', body, token }),
  chainRetry: (id, token) => request(`/chain/ledger/${id}/retry`, { method: 'POST', token }),
  chainReconcile: (id, token) =>
    request(`/chain/ledger/${id}/reconcile`, { method: 'POST', token }),
  chainAwaitingCommittee: (token) => request('/chain/awaiting-committee', { token }),
  chainFinalizeIx: (id, authority, token) =>
    request(`/chain/ledger/${id}/finalize-ix${query({ authority })}`, { token }),
  chainSquadsProposal: (id, params, token) =>
    request(`/chain/ledger/${id}/squads-proposal${query(params)}`, { token }),

  walletChallenge: (token) => request('/wallet/challenge', { method: 'POST', token }),
  walletLink: (body, token) => request('/wallet/link', { method: 'POST', body, token }),
  walletMe: (token) => request('/wallet/me', { token }),
};
