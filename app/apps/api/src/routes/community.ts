...
import { explorerAccountUrl, explorerTxUrl, isLikelyFakeSignature } from "@kna/chain-client";
...
function withExplorer(
  attestation: {
    state: string;
    payloadHash: string;
    pendingPda: string | null;
    finalPda: string | null;
    pendingTxSig: string | null;
    finalizeTxSig: string | null;
  } | null
) {
  if (!attestation) return null;
  const cluster = loadChainConfig().cluster;
  return {
    ...attestation,
    explorer: {
      pending:
        attestation.pendingTxSig && !isLikelyFakeSignature(attestation.pendingTxSig)
          ? explorerTxUrl(cluster, attestation.pendingTxSig)
          : null,
      finalize:
        attestation.finalizeTxSig && !isLikelyFakeSignature(attestation.finalizeTxSig)
          ? explorerTxUrl(cluster, attestation.finalizeTxSig)
          : null,
      pendingPda: attestation.pendingPda
        ? explorerAccountUrl(cluster, attestation.pendingPda)
        : null,
      finalPda: attestation.finalPda
        ? explorerAccountUrl(cluster, attestation.finalPda)
        : null,
    },
  };
}
...
// and publishing it as one is the thing that would most undermine
...
communityRouter.get("/ledger", async (req, res) => {
  ...
  res.json(
    entries.map((entry) => ({
      ...entry,
      attestation: withExplorer(entry.attestation),
    }))
  );
});
